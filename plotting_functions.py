...
from lineage_bias import calc_bias
...
def plot_contributions(
    ...
    first_time_df = find_first_clones_in_mouse(
        clonal_abundance_df,
        timepoint_col
    )
    first_time_df[timepoint_col] = 'first'
    contributions = percentile_sum_engraftment(first_time_df, timepoint_col, cell_type=cell_type)
    exp_x, exp_y = find_intersect(
        data=contributions,
        y=50.0,
    )
    plot = sns.lineplot(
...
def swamplot_abundance_cutoff(
    ...
    filtered_df = filter_mice_with_n_timepoints(
        filtered_df,
        nunique_time
    )
...
def plot_not_survived_abundance(
    ...
    if by_average:
        abund_df = add_avg_abundance_until_timepoint_clonal_abundance_df(
            clonal_abundance_df,
            timepoint_col
        )
        y_col = 'avg_abundance'
...
def plot_n_most_abundant(
    ...
    all_timepoints = filter_mice_with_n_timepoints(
        clonal_abundance_df,
...
def plot_clone_count_swarm(
    ...
    no_6_or_17_month_df = remove_month_17_and_6(
        clonal_abundance_df,
        timepoint_col
    )
    nunique_times = no_6_or_17_month_df[timepoint_col].nunique()
    filt_clonal_abundance_df = filter_mice_with_n_timepoints(
        no_6_or_17_month_df,
        nunique_times
    )
    threshold_df = filter_cell_type_threshold(
...
def plot_swarm_violin_first_last_bias(
    ...
    no_m17_df = remove_month_17(
        lineage_bias_df,
        timepoint_col
    )
    first_clones = find_first_clones_in_mouse(
        lineage_bias_df,
        timepoint_col
    ).assign(time_desc='First')
    last_clones = find_last_clones_in_mouse(
        lineage_bias_df,
        timepoint_col
...
def exhaust_persist_abund(
    ...
    exhaustion_labeled_df = label_exhausted_clones(
        None,
        clonal_abundance_df,
        timepoint_col
    )
...
def exhaust_persist_hsc_abund(
    ...
    Arguments:
        ...
        clonal_abundance_df {pd.DataFrame}
        timepoint_col {str}
        min_hsc_per_mouse {pd.DataFrame} -- Dataframe of minimum HSC
            abundance required for one hsc clone to be 'real'
    ...
    if timepoint_col == 'gen':
        ...
        last_only = False
    survival_with_hsc_df = label_exhausted_clones(
        hsc_data,
        clonal_abundance_df,
        timepoint_col,
    ...
    if by_sum:
        ...
        survival_hsc_df = fill_mouse_id_zeroes(
            survival_hsc_df,
            info_cols=['group'],
            fill_col=y_col,
...
def heatmap_correlation_hsc_ct(
    ...
    filt_abundance = remove_month_17_and_6(
        clonal_abundance_df,
        timepoint_col,
    )
    ...
    with_hsc_data_df = with_hsc_data_df[['mouse_id', 'group', 'code', timepoint_col, 'cell_type', 'hsc_percent_engraftment', 'percent_engraftment']].dropna(axis='index')
    with_hsc_data_df = remove_month_17_and_6(
        with_hsc_data_df,
        timepoint_col
    )
...
def plot_dist_bias_at_time_vs_group_facet_grid(
    ...
    center_points = (bin_edges[1:] + bin_edges[:-1]) / 2
    dodged_center_points = center_points + dodge_amount
    lineage_bias_df = remove_month_17(
        lineage_bias_df,
        timepoint_col
    )
    desc_addon = ''
...
def plot_abundance_change_changed_group_grid(
    ...
    marked_bias_change_df = mark_changed(
        bias_change_df,
        bias_change_df,
        min_time_difference=mtd,
    )
...
def plot_hsc_and_blood_clone_count(
    ...
    last_blood_data = get_clones_at_timepoint(
        blood_data,
        timepoint_col,
        'last',
        by_mouse=True,
...
def plot_hsc_vs_cell_type_abundance_bootstrapped(
    ...
    if by_group:
        totals_in_out = fill_mouse_id_zeroes(
            totals_in_out,
            info_cols=['group'],
            fill_col='code',
...
def plot_abundance_changed_bygroup(
    ...
    change_marked_df = remove_month_17_and_6(
        change_marked_df,
        timepoint_col
    )
    fname_prefix = save_path + os.sep \
...
def plot_abundance_bias_change_type_heatmap(
    ...
    bias_with_cats = add_bias_category(
        lineage_bias_df
    )
    change_marked_df = remove_month_17_and_6(
        change_marked_df,
    ...
    only_n_timepoint_mice = filter_mice_with_n_timepoints(
        change_marked_df,
        n_timepoints=4,
    )
    ...
    only_change_type_df = add_short_group(
        only_change_type_df
    )
    order =[
        ('LB', 'E'),
    ...
    if plot_average:
        y_col = 'avg_abundance'
        only_change_type_df = add_avg_abundance_until_timepoint_clonal_abundance_df(
            only_change_type_df,
            timepoint_col
        )
...
def plot_balanced_clone_abundance(
    ...
    for time, t_df in balanced_clones.groupby(timepoint_col):
        filt_t_df = filter_lineage_bias_thresholds(
            t_df,
            thresholds,
        )
...
def plot_percent_balanced_expanded(
    ...
    mouse_time = filt_time_abundance[['mouse_id', timepoint_col, 'group']].drop_duplicates()
    expanded_clones = filter_cell_type_threshold(
        filt_time_abundance,
        thresholds,
        analyzed_cell_types=thresholds.keys(),
    )
    bias_cat_df = add_bias_category(lineage_bias_df)
    expanded_bias_df = bias_cat_df.merge(
        expanded_clones[['code', 'mouse_id', timepoint_col]].drop_duplicates(),
        how='inner',
        validate='1:1'
    )
    bias_counts = pd.DataFrame(
        expanded_bias_df \
            .groupby(['mouse_id', timepoint_col, 'group', 'bias_category']) \
            .code.nunique()
    ).reset_index().rename(
        columns={'code': '# of Expanded Clones'}
    )
    totals = pd.DataFrame(
        bias_counts.groupby(['mouse_id', timepoint_col])\
...
def plot_count_biased_changing_at_time(
    ...
    with_bias_cat = add_bias_category(change_marked_df)
    ...
    for i in range(2):
        ...
        cat_zero_df = fill_mouse_id_zeroes(
            cat_df,
            info_cols=['group'],
            fill_col='# of Clones',
...
def cell_type_expanded_hsc_vs_group(
    ...
    expanded_at_time_df = filter_threshold(
        timepoint_df,
    ...
    hsc_data = filt_df[filt_df.cell_type == 'hsc'] 

    expanded_time_clones = expanded_at_time_df[['mouse_id', 'code']].drop_duplicates()

    expanded_time_hsc = hsc_data.merge(
    ...
    zero_filled = hsc_abundance_per_mouse.merge(
        mice_with_data,
        how='outer',
        validate='1:1'
    ).fillna(0)
    zero_filled['time'] = timepoint
    ...
    ax.set_ylabel('Mean HSC Abundance')
