...
def test_baseline_missing_value():
    ...
    try:
        calculate_baseline(test_data, baseline_timepoint=4, baseline_column='month')
        assert False
    except ValueError:
        assert True
...
def test_norm_missing_baseline_value():
    ...
    test_data = TEST_LONG_DATA.loc[TEST_LONG_DATA.mouse_id == 'M4']
    try:
        normalize_input(test_data, baseline_timepoint=4, baseline_column='month', analyzed_cell_types=['gr', 'b'])
        assert False
    except ValueError:
        assert True
